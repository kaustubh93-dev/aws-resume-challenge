...
```bash
aws-resume-challenge$ sam build --use-container
```
...
```bash
aws-resume-challenge$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
aws-resume-challenge$ sam local start-api
aws-resume-challenge$ curl http://localhost:3000/
```
...
```bash
aws-resume-challenge$ sam logs -n HelloWorldFunction --stack-name "aws-resume-challenge" --tail
```
...
```bash
aws-resume-challenge$ pip install -r tests/requirements.txt --user
# unit test
aws-resume-challenge$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
aws-resume-challenge$ AWS_SAM_STACK_NAME="aws-resume-challenge" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "aws-resume-challenge"
```
